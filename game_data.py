class Location:
    ...
    def __init__(self,longdes,shortdes,index,score,visit=False):
        ...
        self.command = ["look", "inventory", "score", "quit", "back"]
        #self.x = x
        #self.y = y
        self.index = index
        self.score = score
...
class World:
    ...
    def __init__(self, mapdata, locdata, itemdata):
        ...
        self.loclst = self.load_locations(locdata) # This data must be stored somewhere. Up to you how you choose to do it...
        self.item = self.load_items(itemdata) # This data must be stored somewhere. Up to you how you choose to do it...
    ...
    def load_map(self, filename):
        ...
        self.nestlst = []
        mapdata = open(filename,'r')
        for i in mapdata:
            self.nestlst.append(i.split())
        for i in range(0,len(self.nestlst)):
            for j in range(0,len(self.nestlst[i])):
                self.nestlst[i][j] = int(self.nestlst[i][j])
        return self.nestlst
    ...
    def load_locations(self, filename):
        ...
        locationdata=open(filename,'r')
        finlst=[]
        for i in locationdata:
            if i !=" ":
                index=locationdata.readline().strip("LOCATION ")#index
                score=locationdata.readline()#score
                SD=locationdata.readline()#S.D
                LD=locationdata.readline() #L.D
                blank=locationdata.readline()
                finlst.append(Location(LD, SD, index, score))

        # for i in finlst:
        #     print (i.index)
        #     print (i.score)
        #     print (i.get_brief_description())
        #     print (i.get_full_description())
        return finlst
    ...
    def get_location(self, x, y):
        ...
        self.map = self.nestlst
        if y <= len(self.map)-1 and x <= len(self.map[y]):
            if self.map[y][x] != -1:
                return self.map[y][x]
